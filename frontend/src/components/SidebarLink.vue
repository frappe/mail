<template>
    <button v-if="link && !link.onlyMobile"
        ...
        <div class="flex items-center w-full duration-300 ease-in-out group" :class="isCollapsed ? 'p-1' : 'px-2 py-1'">
            <Tooltip :text="link.label" placement="right">
                <slot name="icon">
                    <span class="grid h-5 w-6 flex-shrink-0 place-items-center">
                        <component :is="icons[link.icon]" class="h-4 w-4 stroke-1.5 text-gray-800" />
                    </span>
                </slot>
            </Tooltip>
            ...
            <div v-if="showControls"
                ...
                <component :is="icons['Edit']" class="h-3 w-3 stroke-1.5 text-gray-800" @click.stop="openModal(link)" />
                <component :is="icons['X']" class="h-3 w-3 stroke-1.5 text-gray-800" @click.stop="deletePage(link)" />
            </div>
        </div>
    </button>
</template>
