...
class MailPrincipal(Document):
	...
	def db_insert(self, *args, **kwargs) -> None:
		self.name = MailPrincipal._create(self, ignore_permissions=bool(self.flags.ignore_permissions))

	def load_from_db(self) -> "MailPrincipal":
		principal = MailPrincipal._get(self.name)
		return super(Document, self).__init__(principal)

	def db_update(self) -> None:
		MailPrincipal._update(self)
		self.reload()

	def delete(self) -> None:
		MailPrincipal._delete(self.name)
	...
	def rotate_dkim_keys(self) -> None:
		...
		MailPrincipal._delete_dkim_signature(backend, self.name, "rsa-sha256", raise_exception=False)
		MailPrincipal._create_dkim_signature(backend, self.name, "rsa-sha256", raise_exception=False)

		if bool(frappe.conf.enable_ed25519_dkim):
			MailPrincipal._delete_dkim_signature(backend, self.name, "ed25519-sha256", raise_exception=False)
			MailPrincipal._create_dkim_signature(backend, self.name, "ed25519-sha256", raise_exception=False)
	...
	def sync_jmap_identities(self) -> None:
		...
		MailPrincipal._sync_jmap_identities(self.name)

	@staticmethod
	def _create(principal: "MailPrincipal", ignore_permissions: bool = False) -> str:
		...
		if response.json().get("error"):
			frappe.throw(
				_("Failed to add principal {0}: {1}").format(frappe.bold(principal.name), response.text)
			)
		...
		if principal.type == "Domain":
			try:
				MailPrincipal._create_dkim_signature(
					backend, principal.name, "rsa-sha256", raise_exception=False
				)
	...
	@staticmethod
	def _sync_jmap_identities(pname: str) -> None:
		...
		if not frappe.db.exists(
			...
			{"tenant": tenant, "principal_name": pname, "principal_type": "Individual"},
		):
			return
	...
	@staticmethod
	def _get(pname: str, skip_dns_records: bool = False, ignore_cache: bool = False) -> dict:
		tenant = get_principal_tenant(pname)
		ensure_access_to_tenant(tenant)
		ensure_tenant_has_cluster(tenant)
		...
		principal = MailPrincipal._fetch(
			backend, pname, skip_dns_records=skip_dns_records, ignore_not_found=False
		)
	...
	@staticmethod
	def _update(principal: "MailPrincipal") -> None:
		if principal.type in ["API Key", "Group", "OAuth Client", "Role"]:
			raise NotImplementedError(f"Principal type {principal.type} is not supported yet.")
		...
		existing_principal = frappe._dict(
			MailPrincipal._fetch(backend, principal.name, skip_dns_records=True, ignore_not_found=False)
		)
		...
		response = backend.request(
			"PATCH", f"{PRINCIPAL_ENDPOINT}/{principal.name}", data=json.dumps(actions)
		)
		...
		try:
			MailPrincipal._sync_jmap_identities(principal.name)
		except Exception:
			frappe.log_error(
