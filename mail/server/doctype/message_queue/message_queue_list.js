...
function add_pause_resume_buttons(listview) {
	...
	frappe.call({
		...
		callback: (r) => {
			if (!r.exc) {
				update_pause_resume_buttons(listview, r.message)
			}
		},
	})
}

function update_pause_resume_buttons(listview, status) {
	if (status) {
		listview.page.remove_inner_button('Resume')
		listview.page.add_inner_button(__('Pause'), () => {
			pause_queue(listview)
		})
	} else {
		listview.page.remove_inner_button('Pause')
		listview.page.add_inner_button(__('Resume'), () => {
			resume_queue(listview)
		})
	}
}

function pause_queue(listview) {
	frappe.call({
		...
		callback: (r) => {
			if (!r.exc) {
				update_pause_resume_buttons(listview, false)
			}
		},
	})
}

function resume_queue(listview) {
	frappe.call({
		...
		callback: (r) => {
			if (!r.exc) {
				update_pause_resume_buttons(listview, true)
			}
		},
	})
}
